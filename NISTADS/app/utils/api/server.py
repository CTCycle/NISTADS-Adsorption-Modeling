...
class GetServerStatus:
    ...
    def check_status(self) -> None:
        ...
        if response.status_code == 200:
            logger.info(
                f"NIST server is up and running. Status code: {response.status_code}"
            )
        else:
            logger.error(
        ...
        async with self.semaphore:
            async with session.get(url) as response:
                if response.status != 200:
                    logger.error(
                        f"Could not fetch data from {url}. Status: {response.status}"
                    )
                    return None
                try:
        ...
        async with ClientSession() as session:
            ...
            with tqdm(total=total, desc="Fetching", unit="req") as pbar:
                for idx, future in enumerate(asyncio.as_completed(tasks)):
                    ...
                    update_progress_callback(
                        idx + 1, total, kwargs.get("progress_callback", None)
                    )
                    pbar.update(1)
        return results
...
class GuestHostDataFetch:
    ...
    def get_materials_index(self) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
        ...
        if guest_json.status_code == 200:
            ...
            logger.error(
                f"Failed to retrieve adsorbents data. Status code: {guest_json.status_code}"
            )
        if host_json.status_code == 200:
            ...
            logger.error(
                f"Failed to retrieve adsorbates data. Status code: {host_json.status_code}"
            )
