...
class BuildAdsorptionDataset:
    ...
    def extract_nested_data(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        dataframe["adsorbent_name"] = (
            dataframe["adsorbent"].apply(lambda x: x["name"].lower()).astype(str)
        )
        dataframe["adsorbates_ID"] = (
            dataframe["adsorbates"]
            .apply(lambda x: [f["InChIKey"] for f in x])
            .astype(str)
        )
        dataframe["adsorbate_name"] = (
            dataframe["adsorbates"]
            .apply(lambda x: [f["name"].lower() for f in x])
            .astype(str)
        )
        ...
        # check if the number of guest species is two (binary mixture dataset)
            ...
            dataframe["compound_1_pressure"] = dataframe.apply(
                lambda x: [
                    a * b
                    for a, b in zip(x["compound_1_composition"], x["total_pressure"])
                ],
                axis=1,
            )
            dataframe["compound_2_pressure"] = dataframe.apply(
                lambda x: [
                    a * b
                    for a, b in zip(x["compound_2_composition"], x["total_pressure"])
                ],
                axis=1,
            )
